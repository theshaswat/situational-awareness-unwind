...
**The book screened liquid on a standard measure, and the one flag it produced was the one position that actually failed to clear:**
...
- Recovered trade-level forced-sale evidence from the Schedule 13D/A Exhibit
  99.2 transaction blotter on Core Scientific, with dates, prices and share counts
...
Option entries disclose the value of the underlying only, with no strike,
...
committed raw SEC filings and asserts the published outputs reproduce, that
the headline figures quoted in this README still match the committed tables,
and that the two related filer CIKs have not been summed into a double-counted
book. It runs in CI on every push.

CSV outputs are compared byte-for-byte. Parquet and PNG are compared by
content instead, because both carry platform-dependent encoding (parquet
writer metadata, matplotlib font rasterisation) that differs between a macOS
workstation and a Linux runner while the data itself is unchanged.
